...
StaticMesh::StaticMesh(ID3D11Device * device, ID3D11DeviceContext * deviceContext, ShaderSystem* shaderSystem)
{
	m_device = device;
...
bool StaticMesh::LoadMesh(const std::string& filename,  bool isRHCoordSys, bool computeNormals)
{
	HRESULT hr = 0;
	...
	else	//If we could not open the file
	{
		...
		MessageBox(0, L"Could not Open Obj File",	//display message
			L"Error", MB_OK);
	...
	std::wstring dir = L"../Asset/";

	meshMatLib = dir + meshMatLib;
							...
							if (checkChar == 'K')
							{
								...
								if (checkChar == 'd')
								{
									...
									if (!alreadyLoaded)
									{
										...
										hr = LoadTexture(m_device, m_deviceContext, fileNamePath.c_str(), &tempMeshSRV);
	...
	hr = m_device->CreateBuffer(&vertexBufferDesc, &vertexBufferData, &m_vertexBuffer);
	...
	hr = m_device->CreateBuffer(&cbbd, NULL, &cbPerFrameBuffer);
...
void StaticMesh::Shutdown()
{
	...
	for (auto texture : meshSRV)
	{
		texture->Release();
	}
}

void StaticMesh::Render(float frameTime, XMMATRIX & worldMatrix, XMMATRIX & viewMatrix, XMMATRIX & projectionMatrix)
{


	constbuffPerFrame.light = light;
	m_deviceContext->UpdateSubresource(cbPerFrameBuffer, 0, NULL, &constbuffPerFrame, 0, 0);
	m_deviceContext->PSSetConstantBuffers(0, 1, &cbPerFrameBuffer);


	m_shaderSystem->Render(COLOR_SHADER);
	...
	for (int i = 0; i < meshSubsets; ++i)
	{
		...
		cbPerObj.difColor = material[subsetMaterialArray[i]].difColor;
		cbPerObj.hasTexture = material[subsetMaterialArray[i]].hasTexture;
		...
		m_deviceContext->PSSetConstantBuffers(1, 1, &cbPerObjectBuffer);


		//if (material[subsetMaterialArray[i]].hasTexture)
		//	m_deviceContext->PSSetShaderResources(0, 1, &meshSRV[material[subsetMaterialArray[i]].texArrayIndex]);
		...
		if (!material[subsetIndexStart[i]].transparent)
			m_deviceContext->DrawIndexed(indexDrawAmount, indexStart, 0);
	}
}
...
void StaticMesh::Initialize()
{
	...
	m_device->CreateSamplerState(&sampDesc, &m_sampleState);
