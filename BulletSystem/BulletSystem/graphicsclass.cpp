...
bool GraphicsClass::Render()
{
	...
	for (auto& bulletSystem : m_bulletSystem)
	{
		bulletSystem->SetTarget(-100, -100);
		bulletSystem->SetPosition(10, 10, 0);
		Point startPosition = bulletSystem->GetPosition();
		XMMATRIX startPositionMatrix = XMMatrixTranslation(startPosition.x, startPosition.y, startPosition.z);

		bulletSystem->Update(elapsedTime);//업데이트

		for (auto& bullet : bulletSystem->bullets)//렌더
		{
